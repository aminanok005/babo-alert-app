...
async function pollStatus() {
    ...
    try {
        ...
        if (data.status === 'completed') {
            ...
            const btn = document.getElementById('generateBtn');
            btn.disabled = false;
            btn.innerHTML = '<i class="fas fa-play"></i> 🎬 Generate 7 คลิป';
        } else if (data.status === 'failed') {
            clearInterval(pollInterval);
            const errorMsg = data.error || 'เกิดข้อผิดพลาดที่ไม่ทราบสาเหตุ';
            alert('การสร้างคลิปล้มเหลว: ' + errorMsg);
...
async function showContext(type, event) {
    // Update active tab
...
function resetApp() {
    ...
    document.getElementById('progressText').textContent = '0/7 คลิป';
